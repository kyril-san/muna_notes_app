...
class FirebaseCloudService implements CloudProvider {
  ...
  Future<Iterable<CloudNote>> getNotes({required String getuserid}) async {
    final getallnotes = notes
        ...
        .get()
        .then((value) => value.docs.map((e) => CloudNote.fromFirebase(e)));

    //! tried experiment to see if I could still use a stream<Iterable<Cloudnote>> here
    ...
    // final allnotes = getallnotes.asyncExpand((event) async* {
    //   yield event.docs.map((e) => CloudNote.fromFirebase(e));
    // });
